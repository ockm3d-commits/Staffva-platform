/**
 * Apply migration 00081 — profile_edit_requests table + recruiter_messages extension.
 *
 * Creates the profile_edit_requests table (per-field candidate edit approval
 * flow), extends recruiter_messages with message_type + edit_request_id, and
 * installs RLS policies for candidate/recruiter access.
 *
 ...
 *
 * Run: npx tsx --env-file=.env.local scripts/apply-migration-081.ts
 */
...
async function main() {
  const migrationPath = join(
    process.cwd(),
    "supabase/migrations/00081_profile_edit_requests.sql"
  );
  console.log("════════════════════════════════════════════════════════════");
  console.log("Applying migration 00081");
  ...
  console.log("\nNext: run the Phase 1 verification queries to confirm table,");
  console.log("columns, RLS, policies, and indexes are in place.");
}
